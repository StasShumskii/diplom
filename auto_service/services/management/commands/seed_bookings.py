...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if existing_bookings > 0:
            self.stdout.write(self.style.WARNING(f'Deleting {existing_bookings} existing bookings...'))
        ...
        try:
            ...
            if not services:
                self.stdout.write(self.style.ERROR('No services found. Please run seed_data first.'))
                return
            ...
            if not service_names:
                service_names = [
                    'Заміна моторної оливи', 'ТО-1', 'ТО-2', 'Діагностика',
                    'Заміна гальмівних колодок', 'Розвал-сходження',
                    'Заміна амортизаторів', 'Заправка кондиціонера',
                    'Заміна повітряного фільтра', 'Заміна салонного фільтра',
                    'Заміна гальмівних дисків', 'Заміна акумулятора'
                ]
