    ...
    for brand_name, models in brands_data.items():
        ...
        for m_name in models:
            m, _ = CarModel.objects.get_or_create(
                brand=brand, name=m_name, defaults={"year_start": 2015}
            )
            all_models.append(m)

    mngr, _ = User.objects.get_or_create(
        email="manager@gmail.com",
        defaults={"username": "manager", "role": "MANAGER", "is_staff": True},
    )
    mngr.set_password("admin123")
    mngr.save()

    client, _ = User.objects.get_or_create(
        email="client@gmail.com", defaults={"username": "client", "role": "CLIENT"}
    )
    client.set_password("pass123")
    client.save()
    clients = [client]

    mech, _ = User.objects.get_or_create(
        username="mechanic", defaults={"role": "MECHANIC", "is_staff": True}
    )
