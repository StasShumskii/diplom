...
def add_review(request):
    ...
    if rating and title and comment:
        Review.objects.create(user=request.user, rating=rating, title=title, comment=comment)
        ...
        return JsonResponse(
            {"status": "success", "message": "Дякуємо за відгук! Вам нараховано 50 балів!"}
        )
...
def get_chat_messages(request):
    messages = ChatMessage.objects.filter(user=request.user).order_by("created_at")
    ChatMessage.objects.filter(user=request.user, is_from_admin=False, is_read=False).update(
        is_read=True
    )
