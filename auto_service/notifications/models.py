...
class Review(models.Model):
    ...
    car = models.ForeignKey(
        UserCar, on_delete=models.SET_NULL, null=True, blank=True, related_name="reviews"
    )
    rating = models.PositiveIntegerField(
        choices=[(i, i) for i in range(1, 6)], verbose_name=_("Оцінка")
    )
...
class Reminder(models.Model):
    ...
    reminder_type = models.CharField(
        max_length=20, choices=ReminderType.choices, verbose_name=_("Тип нагадування")
    )
    title = models.CharField(max_length=200, verbose_name=_("Заголовок"))
    message = models.TextField(verbose_name=_("Повідомлення"))
    reminder_date = models.DateTimeField(verbose_name=_("Дата нагадування"))
    mileage_threshold = models.PositiveIntegerField(
        blank=True, null=True, verbose_name=_("Пробіг для нагадування")
    )
